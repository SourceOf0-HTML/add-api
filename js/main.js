...
// Flickr API key
var apiKey = "b26fffccf80fd8f0bec9e7be41fb4c67";
...
// FlickerのAPIから写真取得
function createFlickrImage(word) {
  ...
  $.getJSON(flickr_url, function(data){
    console.log(data);
    if (data.stat == "ok") {
      // 画像を追加する.image-galleryの要素を$divにセット
      var $div = $(".image-gallery");

      for (var i = 0; i < data.photos.photo.length; i++) {
        var photo = data.photos.photo[i];
        var photoText = getFlickrText(photo);

        // $divに <div class="image-gallery__item"><a href="..." ...><img src="..." ...></a></div> を追加する
        $div.append(
          $("<div>", {
...
$(document).ready(function() {
  ...
  $(".animated").waypoint(function(direction) {
    if (direction === "down") {
      $(this.element).removeClass("fadeOutUp").addClass("fadeInUp");
    } else if (direction === "up") {
      $(this.element).removeClass("fadeInUp").addClass("fadeOutUp");
    }
  }, { offset: "50%" });
  ...
  // Flicker初期化
  createFlickrImage("cat");
  createFlickrImage("dog");
